# Діаграма прецедентів
## Загальна схема
@startuml
  actor Користувач
  actor Редактор
  actor Адміністратор
  
  usecase Реєстрація
  usecase Авторизація
  
  usecase "Редагування існуючих \nданих" as РІД
  usecase "Управління даними \nсистемами" as УДС
  usecase "Управління редакторами \nсистеми" as УРС
  usecase "Взаємодія з \nданими" as ВЗД


  
  
  Редактор -u-|> Користувач
  Адміністратор -u-|> Редактор
  
  Реєстрація <- Користувач
  Користувач -u-> Авторизація
  Користувач -> ВЗД

  
  Редактор -l-> РІД

  УДС <- Адміністратор
  Адміністратор -> УРС
@enduml

## Схема користувача

@startuml
  actor Користувач
  
  
  usecase Реєстрація
  usecase Авторизація
  
  usecase "Взаємодія з \nданими" as ВЗД
  usecase "Запит на пошук \nданих" as ЗНПД
  usecase "Запит на редагування \nданих" as ЗНРД



  
  

  Реєстрація <- Користувач
  Користувач -u-> Авторизація
  Користувач -> ВЗД
  
  ЗНПД -u-> ВЗД
  ВЗД <-u- ЗНРД


  

@enduml


## Схема Редактора
@startuml
  actor Редактор
  
  usecase "Редагування розмітки \nфайлу" as РРФ
  
  Редактор -u-> РРФ

@enduml

## Схема адміністратора

@startuml
  actor Адміністратор
  
  usecase "Видалення \nфайлу" as ВФ
  usecase "Зміна розмітки \nфайлу" as ЗРФ
  usecase "Історія дій над \nфайлом" as ІДНФ
  usecase "Завантаження нових \nданих" as ЗНД
  usecase "Управління даними \nсистеми" as УДС
  usecase "Надання користувачу прав \nредактора" as НКПР
  usecase "Гілка змін \nредактора" as ГЗР
  usecase "Позбавлення користувача \nправ редактора" as ПКПР
  usecase "Управління редакторами \nсистеми" as УРС

  
  Адміністратор -u-> УДС
  ВФ --> УДС
  ЗРФ --> УДС
  ІДНФ --> УДС
  ЗНД --> УДС
  Адміністратор --> УРС
  НКПР --> УРС
  ГЗР --> УРС
  ПКПР --> УРС


@enduml
---


***КОРИСТУВАЧ***

### Створення облікового запису

| **_ID:_**                | NEW_USER_REG                                                      |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Створення облікового запису користувача                                                 |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Відсутність облікового запису користувача                         |
| **_РЕЗУЛЬТАТ:_**         | Запит на змінення маркування файлу, надісланий адміністратору                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | 1. Некоректно введені дані для реєстрації (USER_DATA_ERROR)       |
|                          | 2. Обліковий запис користувача вже існує (ACC_EXIST_ERROR)                              |
 **_ОСНОВНИЙ СЦЕНАРІЙ:_**

@startuml

|Користувач|
start;
: Користувач починає роботу з продуктом;
: Користувач розпочинає реєстрацію;
: Користувач вводить необхідні дані;

|Система|
: Система перевіряє коректність введених даних
<font color="red"><b> USER_DATA_ERROR;
: Система перевіряє наявність зареєстрованого облікового запису
<font color="red"><b> ACC_EXIST_ERROR;
: Система створює новий обліковий запис користувача;

|Користувач|
: Користувач отримує повідомлення про успішну реєстрацію;
: Користувач закінчує роботу з продуктом;

stop;
@enduml
 
### Авторизація користувача

| **_ID:_**                | USER_AUTHORIZATION                                                        |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Авторизація користувача                                            |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Наявність облікового запису                          |
| **_РЕЗУЛЬТАТ:_**         | Авторизація користувача в системі                 |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | 1. Користувач не має облікового запису (ACC_DOESNT_EXIST_ERROR)        |
|                          | 2. Некоректно введені дані (DATA_ENTERED_ERROR)                             |
  **_ОСНОВНИЙ СЦЕНАРІЙ:_**  

@startuml
|Користувач|
start;
: Користувач починає роботу з продуктом;
: Користувач вводить дані для авторизації;
:  Користувач натискає кнопку "Ввійти в обліковий запис";

|Система|
    : Система перевіряє наявність облікового запису
    <font color="red"><b> ACC_DOESNT_EXIST_ERROR;
     :  Система перевіряє коректність введених даних
    <font color="red"><b> DATA_ENTERED_ERROR;
    : Система авторизує користувача;

|Користувач|
: Користувач починає роботу в своєму обліковому записі;
: Користувач закінчує роботу з продуктом;
stop;
@enduml

### Пошук даних

| **_ID:_**                | USER.D_SEARCH                                                        |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Користувач починає пошук даних                                                 |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Користувач повинен увійти в систему                          |
| **_РЕЗУЛЬТАТ:_**         | Користувач отримує результат пошуку за наданим запитом                 |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Дані у системі відсутні (DATA_SYS_MISS)      |
  **_ОСНОВНИЙ СЦЕНАРІЙ:_**  

@startuml
|Користувач|
start;
:  Користувач розпочинає роботу з продуктом;
: Користувач вводить запит на пошук даних;
:  Користувач натискає кнопку "Знайти";

|Система|
    : Система перевіряє наявність інформації у базі даних
    <font color="red"><b> DATA_SYS_MISS;
    : Система виводить на екран результат пошуку; 
    
|Користувач|
: Користувач закінчує роботу з продуктом;
stop;
@enduml

### Запит на редагування даних

| **_ID:_**                | USER.D_EDIT_REQUEST                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Користувач надає запит на редагування даних                                                  |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Користувач повинен увійти в систему                      |
| **_РЕЗУЛЬТАТ:_**         | Користувач отримує дозвіл на редагування даних                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Користувач не може редагувати дані (USER_EDIT_ERROR)     |
  **_ОСНОВНИЙ СЦЕНАРІЙ:_**

@startuml
|Користувач|
start;
: Користувач розпочинає роботу з продуктом;
: Користувач надсилає запит на отримання дозволу редагування даних;

|Система|
    : Адміністратор розглядає запит на дозвіл та підтверджує, або відхиляє його
<font color="red"><b> USER_EDIT_ERROR;

|Користувач|
: Користувач отримує дозвіл на редагування;
: Користувач закінчує роботу з продуктом;
stop;
@enduml

### Візуалізація даних

| **_ID:_**                | USER.D_VISUAL                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Візуалізація даних                                            |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Користувач повинен увійти в систему                         |
| **_РЕЗУЛЬТАТ:_**         | Користувач отримує інтерактивну візуалізацію обраних даних                |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Дані неможливо візуалізувати (DATA_VISUAL_ERROR)        |
  **_ОСНОВНИЙ СЦЕНАРІЙ:_** 

@startuml
|Користувач|
start;
: Користувач розпочинає роботу з продуктом;
: Користувач здійснює пошук даних;
: Користувач обирає дані які бажає візуалізувати;
:Користувач натискає кнопку "Тип візуалізації" та обирає тип який йому потрібен
<font color="red"><b> DATA_VISUAL_ERROR;

|Система|
    :  Система створює інтерактивну візуалізацію на основі обраного типу;
    :  Система виводить на екран побудовану інтерактивну візуалізацію даних;
    
|Користувач|
: Користувач закінчує роботу з продуктом;
stop;
@enduml

### Вихід з облікового запису

| **_ID:_**                | USER_EXIT                                                     |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Користувач бажає вийти з облікового запису                                                |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Користувач повинен бути авторизованим                           |
| **_РЕЗУЛЬТАТ:_**         | Вихід з облікового запису                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Відсутні        |
  **_ОСНОВНИЙ СЦЕНАРІЙ:_**

@startuml
|Користувач|
start;
: Користувач розпочинає роботу з продуктом;
: Користувач натискає кнопку "Вийти з облікового запису";
:  Користувач отримує повідомлення чи дійсно він бажає вийти з облікового запису;
: Користувач підтверджує вихід;

|Система|
    : Система деавторитизує користувача з системи та закриває доступ до його особистих даних;
    : Система переадресовує користувача на сторінку авторизації;

|Користувач|
: Користувач закінчує роботу з продуктом;


stop;
@enduml

***АДМІНІСТРАТОР***

### Завантаження нових даних

| **_ID:_**                | DATE_UPLOAD                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Завантаження нових даних                                                |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                          |
| **_РЕЗУЛЬТАТ:_**         | Завантаження нових даних у базу даних (систему)              |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Дані вже існують в системі (DATE_EXIST)        |

@startuml
|Адміністратор|
start;
: Адміністратор входить в систему;
: Адміністратор натискає кнопку "Завантажити дані";
:  Адміністратор обирає дані, які треба завантажити;

|Система|
    : Система перевіряє наявність цих даних у базі
                <font color="red"><b> Можлива помилка DATE_EXIST;
     :  Система підтверджує, що цих даних немає в базі, дає дозвіл на продовження вибору даних;

|Адміністратор|
: Адмінітратор натискає кнопку "Завантажити дані";

|Система|
:  Система завантажує нові дані у базу даних;
stop;
@enduml

### Видалення даних

| **_ID:_**                | DATE_DELETE                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Видалення даних                                                |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                           |
| **_РЕЗУЛЬТАТ:_**         | Видалення старих даних з бази даних                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Даних вже не існує (DATE_DOES_NOT_EXIST)        |

@startuml
|Адміністратор|
start;
: Адміністратор входить в систему;
: Адміністратор натискає кнопку "Видалити дані";
:  Адміністратор обирає дані, які треба видалити;

|Система|
    : Система перевіряє наявність цих даних у базі
    <font color="red"><b> Можлива помилка DATE_DOES_NOT_EXIST;
     :  Система підтверджує, що ці дані є в базі;

|Адміністратор|
: Адмінітратор натискає кнопку "Остаточно видалити дані";

|Система|
:  Система видаляє дані у базу даних;
stop;
@enduml

### Надання прав на редагування даних користувачу

| **_ID:_**                | DATE_EDITOR                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Надання прав на редагування даних користувачу                                 |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                          |
| **_РЕЗУЛЬТАТ:_**         | Користувач отримує права на редагування                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Адміністратор не підтверджує права на редагування (NO_EDITING_RIGHTS)        |

@startuml
|Адміністратор|
start;
: Адміністратор входить в систему;
: Адміністратор заходить в список користувачів, які надали запит на редагування даних;

|Система|
    : Система видає список всіх запитів;

|Адміністратор|
: Адміністратор обирає потрібного користувача;
: Адміністратор напискає на кнопку "Надати права редактора" навпроти імені користувача  
                             <font color="red"><b> (Можлива помилка NO_EDITING_RIGHTS);
|Система|
    : Система надає доступ на редагування;

stop;
@enduml

### Завантаження нових даних

| **_ID:_**                | DATE_UPLOAD                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Завантаження нових даних                                                |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                            |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                          |
| **_РЕЗУЛЬТАТ:_**         | Завантаження нових даних у базу даних (систему)              |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Дані вже існують в системі (DATE_EXIST)        |

@startuml
|Адміністратор|
start;
: Входить в систему;
: Натискає кнопку "Завантажити дані";
: Обирає дані, які треба завантажити;

|Система|
: Перевіряє наявність цих даних у базі
<font color="red"><b> (Можлива помилка DATE_EXIST);
: Підтверджує, що цих даних немає в базі,\n дає дозвіл на продовження вибору даних;

|Адміністратор|
: Натискає кнопку "Завантажити дані";

|Система|
: Завантажує нові дані у базу даних;
stop;
@enduml

### Видалення даних

| **_ID:_**                | DATE_DELETE                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Видалення даних                                                |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                           |
| **_РЕЗУЛЬТАТ:_**         | Видалення старих даних з бази даних                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Даних вже не існує (DATE_DOES_NOT_EXIST)        |

@startuml
|Адміністратор|
start;
: Входить в систему;
: Натискає кнопку "Видалити дані";
: Обирає дані, які треба видалити;

|Система|
: Перевіряє наявність цих даних у базі \n<font color="red"><b> (Можлива помилка DATE_DOES_NOT_EXIST);
: Підтверджує, що ці дані є в базі;

|Адміністратор|
: Натискає кнопку "Остаточно видалити дані";

|Система|
: Видаляє дані у базу даних;
stop;
@enduml

| **_ID:_**                | DATE_EDITOR                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Надання прав на редагування даних користувачу                                 |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                          |
| **_РЕЗУЛЬТАТ:_**         | Користувач отримує права на редагування                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Адміністратор не підтверджує права на редагування (NO_EDITING_RIGHTS)        |

@startuml
|Адміністратор|
start;
: Входить в систему;
: Заходить в список користувачів,\n які надали запит на редагування даних;

|Система|
: Видає список всіх запитів;

|Адміністратор|
: Обирає потрібного користувача;
: Натискає на кнопку "Надати права редактора" \n навпроти імені користувача \n<font color="red"><b> (Можлива помилка NO_EDITING_RIGHTS);

|Система|
: Надає доступ на редагування;

stop;
@enduml

### Втрата прав на редагування даних

| **_ID:_**                | DATE_DELETE_EDITOR                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Втрата прав на редагування даних                                 |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                      |
| **_РЕЗУЛЬТАТ:_**         | Користувач втрачає права на редагуванняння                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Цей користувач не може бути позбавлений прав на редагування (CAN_NOT_DELETE_EDITOR) |

@startuml
|Адміністратор|
start;
: Надає запит на пошук всіх редакторів;

|Система|
 : Видає список всіх редакторів;

|Адміністратор|
: Обирає потрібного редактора;
: Натискає кнопку "Забрати права редактора" \n навпроти імені користувача \n<font color="red"><b> (Можлива помилка CAN_NOT_DELETE_EDITOR);

|Система|
: Забирає доступ на редагування;
stop;
@enduml

***РЕДАГУВАННЯ***

| **_ID:_**                | EDITOR.DATA_LABEL_EDIT                                                    |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             | Редагування маркування даних                                |
| **_УЧАСНИКИ:_**          | Користувач, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Користувач має право на редагування маркування даних                   |
| **_РЕЗУЛЬТАТ:_**         | Запит на змінення маркування файлу, надісланий адміністратору               |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Відхилення запиту користувача на доступ до редагування даних (RQS.DENY) |

@startuml
|Користувач|
start;
: Редагує маркування;

|Система|
: Відпраляє адміністратору запит на редагування \n<font color="red"><b> (Можлива помилка RQS.DENY);
stop;
@enduml

### Втрата прав на редагування даних

| **_ID:_**                | DATE_DELETE_EDITOR                                                         |
| ------------------------ | ------------------------------------------------------------------------------ |
| **_НАЗВА:_**             |  Втрата прав на редагування даних                                 |
| **_УЧАСНИКИ:_**          | Адміністратор, Система                                                           |
| **_ПЕРЕДУМОВИ:_**        | Адміністратор ввійшов в систему                      |
| **_РЕЗУЛЬТАТ:_**         | Користувач втрачає права на редагуванняння                  |
| **_ВИКЛЮЧНІ СИТУАЦІЇ:_** | Цей користувач не може бути позбавлений прав на редагування (CAN_NOT_DELETE_EDITOR) |

@startuml
|Адміністратор|
start;
: Адміністратор надає запит на пошук всіх редакторів;

|Система|
 : Система видає список всіх редакторів;

|Адміністратор|
: Адміністратор обирає потрібного редактора;
: Адміністратор натискає кнопку "Забрати права редактора" навпроти імені користувача
                          <font color="red"><b> Можлива помилка CAN_NOT_DELETE_EDITOR;

|Система|
:  Система забирає доступ на редагування;
stop;
@enduml

</center>

